...
class Concat(Layer):
    def __init__(self, sys_config):
        super(Concat, self).__init__(sys_config)
    ...
    def fprop(self, inputs, inference=False):
        ...
        mem_cc = sum(self.get_mem_cc(inputs.out_dims, inputs.out_dims, inputs.l2_hit_rate_act, l2_hit_rate_wr,
    ...
    def bprop(self, inputs):
        ...
        l2_hit_rate_wr = min(self.hw_cfg.l2_size / (prod(inputs.out_dims) * data_size), 1.0)
        # Reads of two input tensors simulated by out_dims
        mem_cc = sum(self.get_mem_cc(inputs.in_dims, inputs.in_dims, inputs.l2_hit_rate_act, l2_hit_rate_wr,
                                     l3_hit_rate_act=inputs.l3_hit_rate_act))
        cycles = max(alu_cc, mem_cc)
        alu_util_factor = (alu_cc / cycles) * 100
        speedup = 1
